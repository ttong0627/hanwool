...
async function sendMmsWithPhoto(phone: string, message: string, photoUri: string): Promise<void> {
  ...
  try {
    ...
    // 사용자 취소 또는 기기 미지원 — 상태 업데이트는 이미 완료됨
  }
}
...
function DeliveryCompleteModal({ order, onConfirm, onCancel }: CompleteModalProps) {
  ...
  const handleConfirm = async () => {
    if (!photoUri) {
      Alert.alert('사진 필요', '배달 완료 사진을 먼저 촬영해 주세요.')
      return
    }
    setLoading(true)
    onConfirm(photoUri)
      ...
      <View style={modal.overlay}>
        <View style={modal.sheet}>
          ...
          <Text style={modal.sub}>
            {order.customer_name}님 · {order.delivery_address}
          </Text>
          ...
          <Text style={modal.smsNotice}>
            📨 완료 처리 후 고객께 사진과 함께 문자가 발송됩니다
          </Text>
          ...
          <View style={modal.buttons}>
            ...
            <TouchableOpacity
              style={[modal.confirmBtn, !photoUri && modal.confirmDisabled]}
              onPress={handleConfirm}
              disabled={!photoUri || loading}
            >
              {loading
                ? <ActivityIndicator color="white" />
                : <Text style={modal.confirmText}>완료 처리 + 문자 발송</Text>
              }
            </TouchableOpacity>
          </View>
        </View>
...
export function DriverHomeScreen() {
  ...
  const { data: orders, isLoading, refetch } = useQuery({
    ...
    queryFn: () =>
      api.get('/deliveries/route', { params: { route_mode: routeMode } }).then((r) => r.data),
    refetchInterval: 60_000,
  })

  const updateMutation = useMutation({
    mutationFn: ({ orderId, status }: { orderId: number; status: string }) =>
      api.put<StatusResponse>(`/orders/${orderId}/status`, null, { params: { status } })
        .then((r) => r.data),
      ...
      <View style={styles.header}>
        ...
        <View style={styles.modeToggle}>
          {(['A', 'B'] as const).map((m) => (
            <TouchableOpacity
              key={m}
              style={[styles.modeBtn, routeMode === m && styles.modeBtnActive]}
      ...
      <FlatList
        ...
        renderItem={({ item: order }) => (
          <View style={[styles.orderCard, order.status === 'delivered' && styles.orderCardDone]}>
            ...
            {order.items_desc && (
              <Text style={styles.items}>{order.items_desc} ({order.quantity}개)</Text>
            )}

            {/* 배달 완료 사진 썸네일 */}
            {order.status === 'delivered' && order.delivery_photo_url && (
              <Image
                source={{ uri: `http://10.0.2.2:8000${order.delivery_photo_url}` }}
              ...
              <View style={styles.actions}>
                ...
                <TouchableOpacity style={styles.statusBtn} onPress={() => handleStatusUpdate(order)}>
                  <Text style={styles.statusBtnText}>
                    {order.status === 'assigned'  ? '픽업 완료'   :
                     order.status === 'picked_up' ? '출발 📨'     : '완료 📷📨'}
                  </Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            {order.status === 'in_transit' && (
              <TouchableOpacity style={styles.delayBtn} onPress={() => handleDelayed(order)}>
                <Text style={styles.delayBtnText}>지연 알림 문자 발송</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  seqBadge: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#f97316', alignItems: 'center', justifyContent: 'center' },
  seqText: { color: 'white', fontSize: 12, fontWeight: 'bold' },
  orderNo: { fontSize: 13, fontWeight: 'bold', color: '#c2410c' },
  dong: { fontSize: 11, color: '#6b7280' },
  statusBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 12 },
  statusText: { fontSize: 11, fontWeight: '600' },
  customerName: { fontSize: 17, fontWeight: 'bold', color: '#111827', marginBottom: 2 },
  ...
  naviBtn: { flex: 2, backgroundColor: '#FEE2E2', paddingVertical: 8, borderRadius: 8, alignItems: 'center' },
  naviBtnText: { color: '#dc2626', fontWeight: '700', fontSize: 13 },
  callBtn: { flex: 1, backgroundColor: '#EFF6FF', paddingVertical: 8, borderRadius: 8, alignItems: 'center' },
  callBtnText: { color: '#2563eb', fontWeight: '700', fontSize: 13 },
  statusBtn: { flex: 2, backgroundColor: '#f97316', paddingVertical: 8, borderRadius: 8, alignItems: 'center' },
  statusBtnText: { color: 'white', fontWeight: '700', fontSize: 13 },
  delayBtn: { marginTop: 8, backgroundColor: '#fef3c7', paddingVertical: 7, borderRadius: 8, alignItems: 'center', borderWidth: 1, borderColor: '#fde68a' },
  delayBtnText: { color: '#92400e', fontWeight: '600', fontSize: 12 },
})
...
const modal = StyleSheet.create({
  ...
  cancelBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, alignItems: 'center', backgroundColor: '#f3f4f6' },
  cancelText: { fontSize: 16, fontWeight: '600', color: '#374151' },
  confirmBtn: { flex: 2, paddingVertical: 14, borderRadius: 12, alignItems: 'center', backgroundColor: '#f97316' },
